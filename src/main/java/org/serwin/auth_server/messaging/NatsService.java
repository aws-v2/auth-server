...
public class NatsService {
     ...
     */
    public void publish(String subj, Object payload) {
        if (natsConnection == null || natsConnection.getStatus() != Connection.Status.CONNECTED) {
            log.warn("NATS not connected, skipping publish to domain: {}", natsPrefix+subj);
            return;
        }
        log.debug("Publishing event to subject: {} ", subj);
        String subject = String.format("%s",subj);
        try {
